...
from hive_db.orders as orders

join (
      select
        first_name || ' ' || last_name as name,
        *
      from hive_db.customers
) customers
...
join (
    ...
    select
        ...
        collect_list(distinct a.id) as order_ids
    ...
    left outer join cdc_test.payment c
    on a.id = c.orderid
...
left outer join cdc_test.payment payments
